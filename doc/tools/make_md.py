...
# Entry point for the Markdown generator.
def main() -> None:
...
def make_rst_class(class_def: ClassDef, state: State, dry_run: bool, output_dir: str) -> None:
    ...
    with open(
        ...
        generator_github_url = f"https://github.com/Redot-Engine/redot-engine/tree/{git_branch}/doc/tools/make_md.py"
        ...
        if class_def.description is not None and class_def.description.strip() != "":
            ...
            f.write("<!-- classref-introduction-group -->\n\n")
            f.write(make_heading("Description", "-"))

            f.write(f"{format_text_block(class_def.description.strip(), class_def, state)}\n\n")

        if not has_description:
            f.write('<div class="contribute">\n')
            f.write(
                translate("There is currently no description for this class. Please help us by contributing one!")
                + "\n\n"
            )
            f.write("</div>\n\n")
        ...
        if len(class_def.properties) > 0:
            f.write("<!-- classref-reftable-group -->\n\n")
        ...
        if len(class_def.signals) > 0:
            ...
            f.write("<!-- classref-descriptions-group -->\n\n")
        ...
        if len(class_def.enums) > 0:
            ...
            for e in class_def.enums.values():
                ...
                f.write(f'<a id="{enum_anchor}"></a>\n\n')
                self_link = f"[🔗](#{enum_anchor})"
                f.write("<!-- classref-enumeration -->\n\n")
        ...
        if any(not p.overrides for p in class_def.properties.values()) > 0:
            ...
            for property_def in class_def.properties.values():
                ...
                if property_setget != "":
                    f.write("<!-- classref-property-setget -->\n\n")
        ...
        if len(class_def.operators) > 0:
            ...
            for method_list in class_def.operators.values():
                for i, m in enumerate(method_list):
                    ...
                    f.write(f'<a id="{operator_anchor}"></a>\n\n')
                    self_link = f"[🔗](#{operator_anchor})"
        ...
        if len(class_def.theme_items) > 0:
            ...
            for theme_item_def in class_def.theme_items.values():
                ...
                f.write("<!-- classref-themeproperty -->\n\n")
...
def make_enum(t: str, is_bitfield: bool, state: State) -> str:
    ...
    if c in state.classes and e in state.classes[c].enums:
        filename = sanitize_class_name(c, True)
        anchor = f"enum_{sanitize_class_name(c)}_{e}"
        if is_bitfield:
...
def make_method_signature(
    ...
    if qualifiers is not None:
        ...
        qualifier_map = {
            "virtual": '<span class="virtual">virtual</span>',
            "const": '<span class="const">const</span>',
            "vararg": '<span class="vararg">vararg</span>',
            "static": '<span class="static">static</span>',
            "constructor": '<span class="constructor">constructor</span>',
...
def make_heading(title: str, underline: str, l10n: bool = True) -> str:
    ...
    # Map RST underline characters to Markdown heading levels
    heading_levels = {"=": 1, "-": 2, "~": 3, "`": 4}
    level = heading_levels.get(underline, 2)
    prefix = "#" * level
...
def make_footer() -> str:
    # Qualifiers are now handled as inline HTML spans in make_method_signature
    # No footer substitutions needed for Markdown output
    return ""
...
def make_link(url: str, title: str) -> str:
    ...
    if match:
        ...
        if match.lastindex == 2:
            ...
            return f"[{groups[1]}](../{groups[0]}.md{groups[1]}) in [{groups[0]}](../{groups[0]}.md)"
        elif match.lastindex == 1:
            # Doc reference, for example:
            # `Math`
            if title != "":
                return f"[{title}](../{groups[0]}.md)"
            return f"[{groups[0]}](../{groups[0]}.md)"
...
def make_rst_index(grouped_classes: Dict[str, List[str]], dry_run: bool, output_dir: str) -> None:
    with open(
        ...
        f.write("<!-- github_url: hide, allow_comments: False -->\n\n")
        ...
        f.write('<a id="doc_class_reference"></a>\n\n')
...
def format_table(f: TextIO, data: List[Tuple[Optional[str], ...]], remove_empty_columns: bool = False) -> None:
    ...
    for i in range(num_columns):
        if remove_empty_columns and (len(data) == 0 or all(row[i] is None or row[i] == "" for row in data)):
            continue
        header_text = data[0][i] if data[0][i] is not None else ""
        f.write(f"      <th>{header_text}</th>\n")
    f.write("    </tr>\n")
    f.write("  </thead>\n")
    ...
    for row_idx, row in enumerate(data[1:], start=1):
        ...
        for i, text in enumerate(row):
            if remove_empty_columns and (len(data) == 0 or all(r[i] is None or r[i] == "" for r in data)):
                continue
